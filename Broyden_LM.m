...
%%=========================================================================
%% Levenberg-Marquardt repair algorithm
function [new_mutant,FE] = Broyden_LM(problem, x , g ,h , fun)
xmin =  problem.lower_bounds;
xmax =  problem.upper_bounds;
obf.J = @objJ;
obf.F = @objfLM;
opt.Display='off';
new_mutant = x(:);
[x,~,~,~,~,output,~,~] = LevenbergMarquardt(obf,x',xmin,xmax,opt);
if isempty(x)
    FE = 0;
else
new_mutant = x(:);
FE = output.funccount-length(x);
end
end
...
MaxFunEvals=3*n;          %   maximal no of function calls
...
%   objective
obj1 = obj.J;obj2 = obj.F;
objx=@(x,vara)obj1(reshape(x,size(xForm)),vara{:});
objy=@(y,vara)obj2(reshape(transformback(y),size(xForm)),vara{:});
%   Jacobian matter
switch Jacobian
    case 'limit'
        Jacobian_method=2;
